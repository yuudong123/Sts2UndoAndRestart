...
- STS2 내부 필드명 변경: reflection 기반이므로 게임 업데이트에 취약합니다.
- 액션 안정화 타이밍: 카드가 플레이 노드에 남아 있는 시점에 캡처/복원이 걸리면 VFX 잔상이 생길 수 있습니다.
- 카드 비용과 런타임 상태: 모델 필드 복원과 카드 UI 갱신이 함께 맞아야 합니다.
- 유물 스택 표시: 실제 모델 값과 UI 활성 표시가 어긋날 수 있어 복원 후 별도 정리가 필요합니다.
- 보상/런 히스토리: F5와 undo가 통계나 보상 계산에 영향을 주지 않도록 별도 스냅샷을 유지합니다.
